...
- 첫 화면은 빈 자유입력보다 시작 방식 카드와 짧은 템플릿 입력을 기본으로 둔다
- 카드는 결과 방향 강제 선택이 아니라 시작 방식 선택으로 되살린다
...
- 결과 전에 사용자를 질문/승인 버튼 앞에서 멈추게 하는 흐름
...
1. Start
- 자유 입력 중심
- `자율입력`, `프롬프트`, `플랜`, `아키텍처`, `검증 및 리뷰` 카드로 시작
- 카드를 고른 뒤 해당 템플릿이나 자유입력창을 표시

2. Result-first
- `direct_render`, `clarify_first`, `approval_pending` 모두 기본적으로 먼저 결과를 보여줌
- 부족한 정보나 확인할 지점은 결과 안의 판단 근거, notes, 열린 질문으로 남김

3. Result
- 모든 renderer 결과 앞에는 `내가 이해한 요청`, `추천 작업 형태`, `왜 이 방향인지`, `빠진 정보`, `조심해야 할 추측`, `다음에 더 잘 요청하려면`을 짧게 보여준다
- renderer별 결과를 성격에 맞게 표시
- `prompt`에는 학습 패널을 붙일 수 있음
- `review`, `plan`, `architecture`에는 post-result follow-up을 붙일 수 있음
- AI 코딩툴 handoff는 plan 결과 이후의 보조 next step으로만 보여주며, 제품의 주 목적이나 첫 번째 결과처럼 보이게 하지 않는다
...
- 학습 패널은 결과 아래에 붙는다
- notes와 학습 패널은 왜 이 프롬프트 구조가 안정적인지 설명해야 한다
- prompt는 사용자의 요청이 정말 다른 AI에게 붙여넣을 입력을 만드는 일일 때 강하게 사용한다

### plan
- 문제, 대상, 방향, 열린 질문이 읽히는 구조화 문서형 결과
- AI 입문자의 막연한 아이디어, 서비스 구상, MVP 정리는 기본적으로 plan 중심으로 다룬다

### architecture
- 경계, 구성요소, 흐름이 보이는 구조 설계형 결과
- 전문 설계도가 아니라 MVP 대화를 위한 구성요소와 흐름 이해를 돕는 결과로 유지한다

### review-report
- 문제점, 보완점, 다음 수정 포인트가 먼저 보이는 검토형 결과
- 좋은 점, 약한 점, 빠진 전제, 위험한 추측, 개선 우선순위가 드러나야 한다
