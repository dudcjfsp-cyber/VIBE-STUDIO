...
- Vibe Studio는 사용자가 스스로 더 잘 묻고 정리하게 되면 덜 쓰게 될 수 있는 학습 발판이다. 숙련자를 오래 붙잡는 파워툴로 키우는 것이 현재 목표가 아니다.
...
- API key 입력과 provider/model 선택은 `고급 설정` 뒤의 선택적 runtime path다. 입문자 기본 흐름에서 API 연결을 요구하는 인상을 주면 안 된다.
...
- Stage 2 이상의 수정 루프, freeform follow-up instruction, renderer handoff 일반화, agent chain은 현재 제품 방향에서 철회한다.
...
- API key/provider 연결을 기본 시작 조건처럼 전면화하지 않는다.
...
- `codex/stage2-implementation-plan`을 현재 방향의 머지 후보나 다음 작업 기준으로 쓰지 않는다.
...
- `npm.cmd run verify:product-web-boundary`
...
- Remaining local branch outside main: `codex/stage2-implementation-plan` exists only as obsolete reference. Do not merge it into `main`.
...
- Result learning quality pass: improve explanatory copy inside existing result panels without adding new agent flows or advanced-provider requirements.
